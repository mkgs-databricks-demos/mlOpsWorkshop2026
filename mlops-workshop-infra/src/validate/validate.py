...
# MAGIC %pip install --upgrade databricks-sdk mlflow databricks-feature-engineering
...
client.set_model_version_tag(model_name, model_version, f"{tag_prefix}.passed", str(all_passed).lower())
client.set_model_version_tag(model_name, model_version, f"{tag_prefix}.timestamp", str(datetime.now()))

for check_name, result in checks.items():
    if result.get("actual") is not None:
        client.set_model_version_tag(
            model_name, model_version,
            f"{tag_prefix}.{check_name}", f"{result['actual']:.4f}",
        )
    client.set_model_version_tag(
        model_name, model_version,
        f"{tag_prefix}.{check_name}.passed", str(result["passed"]).lower(),
    )
...
# These are sanity gates — promotion thresholds (F1>0.75) are in promote.py
METRIC_THRESHOLDS = {
    "test_f1":        0.50,   # better than random
